...
Для этого в корне репозитория создайте файл окружения `.env` на основе шаблона `.env.example`. Далее в созданном файле необходимо изменить значение переменной `NODE_ENV` на `production`. В переменной `APP_HOST` нужно указать имя хоста, на котором разворачивается приложение. При локальном запуске ее значение можно оставить без изменений. `DB_NAME` и `DB_HOST` должны остаться без изменения. Остальные переменные можно оставить со значениями по умолчанию, либо изменить на строковые значения. Пояснения к переменным:
