...
$ docker run -p 8000:8001 astromis/tritoned_antisui_model-10:v1
```
2. Указать в конфигурационном файле `./zoo/config/triton_services.yml` местонахождение моделей
...
4. Запустить Зоопарк командой
```
$ cd zoo && fastapi app/main.py --port=8888
```
5. Запустить контейнер с базой данных
...
7. Указать в файле `./telegram_bot/create_table.py` параметры базы данных (как минимум, пароль) в словаре на строчке 3.
8. Создать (проверить наличие) таблицу командой
```
$ python telegram_bot/create_table.py
```
9.  Указать в файле `./telegram_bot/bot_aiogram.py` токен бота в строчке 155, а также параметры базы данных (как минимум, пароль) в словаре на строчке 157
10. Запустить бота командой
```
...
## Использование открытой для анализа эмоционального состояния и динамики пользователей 
...
1. Скачиваем локльно репозиторий Tritoned Bert

```
$ git clone https://github.com/psytechlab/tritoned_bert.git
```
2. Устаналиваем зависимости
3. Создаем файл `id2label.json` следующего содержания
```json
{
    "0": "no_emotion", 
    "1": "joy", 
...
5. Убедитесь, что у вас появился Docker-образ `tritoned_cedr_emotion_model:v1`.
...
cedr_emotion:
    ...
    networks:
        - postgres
```
7. Уберите все пункты, кроме `api`, из `services.zoo.depends_on` и добавьте пункт `cedr_emotion`.
8. Сохраните файл.
9. Запустите систему стандартным способом.
