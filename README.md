...
Модуль поиска идентификационной информации [kitoboy-pie](https://github.com/psytechlab/kitoboy-pie) является составной частью библиотеки и может быть легко расширен за счет предоставляемого класса `AbstractIE`. В качестве примера разберем, как можно интегрировать большие языковые модели (БЯМ) в модуль.
...
$ pip install -r requirements.txt
```
3. Создайте файл `llm_ie.py` в папке ./app
4. Вставьте в него следующий код и сохраните его (обратите внимание на токен и эндпоинт в методе `post`):
```python
import requests
import json
...
```
5. В файле ./app/main.py внесите изменения, согласно следующему diff-файлу:
```diff
...
```
6. Убедитесь, что у вас установлена библиотека requests
```bash
...
```
8. Сделайте следующий тестовый запрос (номер паспорта выдуман)
```bash
...
Важно отметить, что код обработчика демонстрационный и не содержит сложной обработки ошибок, поэтому при использовании слабой модели высокая вероятность получить ошибку. В случае ошибки попробуйте отправить запрос несколько раз.
